...
void* nvp_chandlers_init(void)
{
    ...
    if (!ctx->throt.dd.command || !ctx->throt.dn.command || !ctx->throt.uu.command || !ctx->throt.up.command ||
        !ctx->throt.ul.command || !ctx->throt.pt.command || !ctx->throt.md.command || !ctx->throt.mu.command ||
        !ctx->throt.mixratio || !ctx->throt.thrdn || !ctx->throt.thrup)
    {
        ndt_log("navP [error]: nvp_chandlers_init: command not found or created\n");
        goto fail;
    }
    else
    {
        ...
        ctx->throt.atc_is_connected = 0; ctx->throt.thptt = NULL;
...
int nvp_chandlers_update(void *inContext)
{
    ...
    switch ((ctx->ground.xfse.info = ctx->throt.info = ctx->info)->ac_type)
    {
        ...
        case ACF_TYP_GENERIC:
            ...
            {
                ctx->acfspec.y.command = XPLMFindCommand("sim/autopilot/servos_on");
                ctx->otto.disc.cc.name = "sim/autopilot/servos_yawd_off_any";
                ctx->otto.conn.cc.name = "sim/autopilot/servos_on";
                ctx->otto.clmb.rc.init_cl_speed = 93.0f;
                if (ctx->acfspec.y.command)
                {
                    // force-eanble YD before AP
                    REGISTER_CHANDLER(ctx->acfspec.y,
                                      chandler_2ndcb,
                                      1 /* before X-Plane */,
                                      XPLMFindCommand("sim/systems/yaw_damper_on"));
                }
            }
