...
void yfs_data_pageopen(yfms_context *yfms)
{
    if (yfms == NULL)
    {
        return; // no error
    }
    if (yfs_main_newpg(yfms, PAGE_DATA))
    {
        return;
    }
    ...
    yfms->lsks[0][5].cback = yfms->lsks[1][5].cback = (YFS_LSK_f)&yfs_lsk_callback_data;
    yfs_data_pageupdt(yfms); return;
}

void yfs_data_pageupdt(yfms_context *yfms)
{
