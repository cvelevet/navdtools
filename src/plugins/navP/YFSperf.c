...
static void yfs_lsk_callback_perf(yfms_context *yfms, int key[2], intptr_t refcon);

void yfs_perf_pageopen(yfms_context *yfms)
{
    if (yfms == NULL)
    {
        return; // no error
    }
    if (yfs_main_newpg(yfms, PAGE_PERF))
    {
        return;
    }
    ...
    yfms->lsks[0][5].cback = yfms->lsks[1][5].cback = (YFS_LSK_f)&yfs_lsk_callback_perf;
    yfs_perf_pageupdt(yfms); return;
}

void yfs_perf_pageupdt(yfms_context *yfms)
{
